...
export async function POST(resquest) {
    try {
        ...
        if (!usuario) {
            return NextResponse.json({ error: "No se encontró usuario con ese correo" } , {status : 404})
        }

        const passwordVallida = await bcrypt.compare(password, usuario.password)

        if (!passwordVallida) {
            return NextResponse.json({ error: "El usuario o la contraseña son incorrectas" }, {status : 401})
        }
        ...
        console.error({error: 'Sucedio un error al iniciar sesión'})

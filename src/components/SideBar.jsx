        ...
        <aside className={`sidebar-component text-white h-screen flex flex-col fixed top-0 left-0 transition-all duration-300 min-h-screen ${isExpanded ? 'w-52' : 'w-16'}`} onMouseEnter={() => setIsExpanded(true)} onMouseLeave={() => setIsExpanded(false)}>
            ...
            <div className='flex flex-col justify-between h-full w-full'>
                ...
                <button
                    ...
                    className='group flex gap-2 py-3 px-5 items-center text-red-400 group-hover:text-red-600 text-center mb-6 transition-colors duration-200'
                >
                    <IconLogout className={`group-hover:text-red-600 transition-colors duration-200`}/>
                    {isExpanded && <span className='group-hover:text-red-600 transition-colors duration-200'>Salir</span>}
                </button>
            </div>
